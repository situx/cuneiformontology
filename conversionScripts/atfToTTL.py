    ...
    for line in atfsslines:
        if line.startswith("#") or line.startswith("$") or line.startswith("@tablet"):
            ...
            for word in linespl:
                ...
                cdlitabs.add(namespaceprefix+":"+str(currenttabletid)+"_"+currentsideuri+"_line"+replaceNonURIChars(str(currentline))+"_word foaf:image \""+str(namespace)+"images/word/word_"+replaceNonURIChars(str(currentline))+"_"+str(currentwordindex)+"_"+str(tabname)+"_"+str(tabsideid[currentside])+"_"+str(currentside)+".jpg\"^^xsd:anyURI .\n")
                if createRefLinks:
