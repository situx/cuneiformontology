...
for log in logograms:
    if counter<9000 and "ref" in log and log["ref"]!="":
        ttlresult.add("cuneidict:"+cleanString(str(log["logogram"]))+"_word rdf:type lemon:Word .\n")
        ttlresult.add("cuneidict:"+cleanString(str(log["logogram"]))+"_word rdfs:label \"Word: "+str(cleanString(str(log["logogram"]))).replace("_"," ")+"\" .\n")
        ttlresult.add("cuneidict:"+cleanString(str(log["logogram"]))+"_word lemon:form cuneidict:"+cleanString(str(log["logogram"]))+"_wordform .\n")
        ttlresult.add("cuneidict:"+cleanString(str(log["logogram"]))+"_wordform rdf:type lemon:WordForm .\n")
        ttlresult.add("cuneidict:"+cleanString(str(log["logogram"]))+"_wordform rdfs:label \"WordForm: "+str(cleanString(str(log["logogram"]))).replace("_"," ")+"\" .\n")
        ttlresult.add("cuneidict:Sum lemon:entry cuneidict:"+cleanString(str(log["logogram"]))+"_word .\n")
        ttlresult.add("cuneidict:"+cleanString(str(log["origvalue"]).replace("-",""))+"_word rdf:type lemon:Word .\n")
        ttlresult.add("cuneidict:"+cleanString(str(log["logogram"]))+"_word graphemon:isLogogramOf cuneidict:"+cleanString(str(log["origvalue"]).replace("-",""))+"_word .\n")
        ttlresult.add("cuneidict:Akk lemon:entry cuneidict:"+cleanString(str(log["origvalue"]).replace("-",""))+"_word .\n")
        ...
        ttlresult.add("cuneidict:"+cleanString(str(log["origvalue"]).replace("-",""))+"_word rdfs:label \"Word: "+str(log["origvalue"]).replace("-","").replace("\"","").replace("_"," ")+"\" .\n")
        ttlresult.add("cuneidict:"+cleanString(str(log["origvalue"]).replace("-","_"))+"_wordform rdf:type lemon:WordForm.\n")
        ttlresult.add("cuneidict:"+cleanString(str(log["origvalue"]).replace("-","_"))+"_wordform rdfs:label \"WordForm: "+str(log["origvalue"])+"\" .\n")
        if "destvalue" in log:
            ttlresult.add("cuneidict:"+cleanString(str(log["origvalue"]).replace("-",""))+"_word lemon:sense cuneidict:"+cleanString(str(log["origvalue"]).replace("-",""))+"_sense .\n")
...
with open('../dict/akklogogram_dict.ttl', 'w',encoding="utf-8") as f:
    ...
    f.write("graphemon:isLogogramOf rdf:type owl:ObjectProperty .\n")
    ...
    f.write("lemon:form rdf:type owl:ObjectProperty .\n ")  
    f.write("lemon:sense rdf:type owl:ObjectProperty .\n ")  
    f.write("lemon:entry rdf:type owl:ObjectProperty .\n ")        
