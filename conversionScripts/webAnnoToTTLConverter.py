...
for tabname in tabletnames:
    ...
    for side in tabletsides:
        ...
        for key in data:
            ...
            if "body" in data[key]:
                for item in data[key]["body"]:
                    ...
                    if item["purpose"]=="tagging":
                        if "source" in item and "id" in item["source"] and item["source"]["id"]=="http://purl.org/cuneiform/Character":
                            ischaracter=True
                        if "source" in item and "id" in item["source"] and item["source"]["id"]=="http://purl.org/cuneiform/Wedge":
                            iswedge=True
            ...
            if wedgeindex!=None and wedgeindex!="":
                res.write("<"+str(indid)+"> oa:hasBody <"+str(indid)+"_body_wedgeindex> .\n")
            ...
            if key in data3d and "target" in data3d[key] and "selector" in data3d[key]["target"]:
                ...
                if ischaracter or (lineindex!=0 and (wedgeindex==None or wedgeindex==0)):
                    ...
                    res.write("<"+str(indid)+"_target3d_labelselector> rdfs:label \"3D Annotation target labeling selector of Annotation of Wedge at "+str(tabname)+" "+str(tabletside)+" line "+str(lineindex)+" char "+str(charindex)+" wedge "+str(wedgeindex)+" on a 3D Mesh\"@en .\n")
                else:
                    res.write("<"+str(indid)+"_target3d_labelselector> rdfs:label \"3D Annotation target labeling selector of Annotation of Wedge at "+str(tabname)+" "+str(tabletside)+" on a 3D Mesh\"@en .\n")
