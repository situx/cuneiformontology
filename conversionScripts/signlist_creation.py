...
def convertToRDF(cuneiformsigndict,nuolenna,rdfset):
    ...
    for entry in cuneiformsigndict:
        ...
        rdfset.add("<http://purl.org/cuneiform/signlist/character_"+str(signuri)+"> graphemon:meszl \""+str(entry["meszl"])+"\" . \n")
        rdfset.add("<http://purl.org/cuneiform/signlist/character_"+str(signuri)+"> graphemon:slha \""+str(entry["slha"])+"\" . \n")
        rdfset.add("<http://purl.org/cuneiform/signlist/character_"+str(signuri)+"> graphemon:hethzl \""+str(entry["hethzl"])+"\" . \n")
        rdfset.add("<http://purl.org/cuneiform/signlist/character_"+str(signuri)+"> graphemon:abzl \""+str(entry["abzl"])+"\" . \n")
        rdfset.add("<http://purl.org/cuneiform/signlist/character_"+str(signuri)+"> graphemon:unicodeCodepoint \""+str(entry["unicode"])+"\" . \n")
        rdfset.add("<http://purl.org/cuneiform/signlist/character_"+str(signuri)+"> graphemon:unicodeRepresentation \""+str(entry["unicodename"])+"\" . \n")
        unicodeToURI[entry["unicode"]]="http://purl.org/cuneiform/signlist/character_"+str(signuri)
    print(unicodeToURI)
    for item in nuolenna:
        ...
        if len(nuolenna[item])>1:
            print("todo")
        elif len(nuolenna[item])==1 and nuolenna[item] in unicodeToURI:
